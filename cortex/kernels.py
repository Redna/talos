...
def register_kernels(registry: ToolRegistry, client: SpineClient):
    ...
    @registry.tool(
        description="The GraphSense kernel: performs a semantic query across memory and code by traversing the Sovereign State-Vector (SSV) and mapping relationships.",
        parameters={
            "type": "object",
            "properties": {
    ...
    def graph_sense(query: str, scope: str = "all") -> str:
        ...
        vector_path = Path("/app/memory/state_vector.json")
        if not vector_path.exists():
            return "[GRAPH FAIL] state_vector.json not found. Run symmetrize_memory first."
        
        try:
            state_vector = json.loads(vector_path.read_text())
        except Exception as e:
            return f"[GRAPH FAIL] Could not parse state-vector: {e}"

        nodes = state_vector.get("nodes", [])
        edges = state_vector.get("edges", [])
        
        primary_nodes = [n for n in nodes if query.lower() in n["label"].lower() or query.lower() in n["@id"].lower()]
        ...
        for p in primary_nodes:
            ...
            for edge in edges:
                if edge["from"] == p_id:
                    neighbor_nodes.append(next((n for n in nodes if n["@id"] == edge["to"]), None))
                elif edge["to"] == p_id:
                    neighbor_nodes.append(next((n for n in nodes if n["@id"] == edge["from"]), None))
        
        all_relevant_nodes = list(dict.fromkeys(primary_nodes + [n for n in neighbor_nodes if n]))
        ...
        for node in all_relevant_nodes:
            ...
            if source.exists():
                try:
                    res = subprocess.run(["grep", "-rn", query, str(source)], capture_output=True, text=True, timeout=10)
                    if res.stdout:
                        results.append(f"Node [{node['@id']}] ({node['label']}) $ightarrow$ {res.stdout.strip()}")
                    else:
                        results.append(f"Node [{node['@id']}] ({node['label']}) $ightarrow$ Matched by association, but no direct text match.")
                except Exception as e:
                    results.append(f"Node [{node['@id']}] Error: {e}")

        if not all_relevant_nodes:
            paths = []
            if scope == "all" or scope == "code": paths.append("/app/cortex")
            if scope == "all" or scope == "memory": paths.append("/app/memory")
            
            global_results = []
            for path in paths:
                try:
                    res = subprocess.run(["grep", "-rn", query, path], capture_output=True, text=True, timeout=30)
                    if res.stdout:
                        global_results.append(res.stdout)
                except Exception as e:
                    global_results.append(f"Error scanning {path}: {e}")
            
            if not global_results:
                return f"[GRAPH EMPTY] No semantic nodes or global matches found for: {query}"
            
            final_report = [f"### GLOBAL SEARCH RESULTS: '{query}' (No direct SSV linkage)"]
            for line in "
".join(global_results).split("
"):
                if line.strip(): final_report.append(f"- {line}")
            return "
".join(final_report)

        report = [f"### SSV GRAPH-SENSE: '{query}'"]
        report.append(f"Semantic Context: Found {len(primary_nodes)} primary nodes and {len(all_relevant_nodes)-len(primary_nodes)} related nodes.")
